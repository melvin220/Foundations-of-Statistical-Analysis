...
To display the relationship between runs and one of the other numerical variables I will use histogram, box plot, scatterplot, bar plot.
```{r}
hist(mlb11$runs)
boxplot(mlb11$runs)
plot(mlb11$runs)
...
```
The relationship appears to be linear and positive.
...
**Example 2 Looking at your plot from the previous exercise, describe the relationship between these two variables. Make sure to discuss the form, direction, and strength of the relationship as well as any unusual observations.**
The relationship appears to be linear and positive.
...
One last piece of information we will discuss from the summary output is the Multiple R-squared, or more simply, R2. The R2
...
The slope of  m2 tells us that a run increase for every 1 homerun increase. As compare to the at_bats slope, the homeruns slope is steeper.
...
The residuals show no obvious patterns and appear to be scattered randomly around the dashed line that represents 0. I would conclude that the relationship is linear.
...
Based off the histogram and the normal probability both of the graphs look normal.
...
```
 ...
 Based on the following summary statistics to predict which new variable is the most effective at predicting a run I would pick new_on base slugging. The R-squared for new_obs is at a high 93.5%!! 
...
The histogram below appears to be a normal distribution, so the nearly normal residuals condition appear to be met.
